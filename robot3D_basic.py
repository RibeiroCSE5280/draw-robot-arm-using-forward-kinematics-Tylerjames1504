...
def getLocalFrameMatrix(R_ij, t_ij):
    ...
    T_ij = np.block([[R_ij, t_ij], [np.zeros((1, 3)), 1]])

    return T_ij


def forward_kinematics(
    Phi: np.array, L1: float, L2: float, L3: float, L4: float
) -> Union[np.array, np.array, np.array, np.array, np.array]:
    ...
    # Now, let's create a cylinder and add it to the local coordinate frame
    link2_mesh = Cylinder(
        r=0.4, height=L2, pos=((L2 / 2 + r), 0, 0), c="red", alpha=0.8, axis=(1, 0, 0)
    )
    sphere2 = Sphere(r=r).pos(0, 0, 0).color("gray").alpha(0.8)
    ...
    sphere3 = Sphere(r=r).pos(0, 0, 0).color("gray").alpha(0.8)

    # Now, let's create a cylinder and add it to the local coordinate frame
    link3_mesh = Cylinder(
        r=0.4, height=L3, pos=((L3 / 2 + r), 0, 0), c="blue", alpha=0.8, axis=(1, 0, 0)
    )
    ...
    p4 = np.array([[L3 + r], [0.0], [0.0]])  # Frame's origin (w.r.t. previous frame)
...
def main():
    ...
    phi3 = 30  # Rotation angle of part 2 in degrees
